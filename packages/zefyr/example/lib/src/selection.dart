...
class _TextSelectionToolbarState extends State<_TextSelectionToolbar> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final itemDatas = <_ItemData>[
      ...
      _ItemData(widget.handleShare, 'مشاركة'),
    ];
...
class _TextSelectionToolbarItemsRenderBox extends RenderBox
  ...
  void _placeChildren() {
    ...
    var fitWidth = 0.0;
    final navButton = firstChild;
    var overflowHeight = overflowOpen && !isAbove ? navButton.size.height : 0.0;
    visitChildren((RenderObject renderObjectChild) {
      i++;
...
class AppTextSelectionControls extends TextSelectionControls {
  ...
  Widget buildToolbar(
    ...
    const closedToolbarHeightNeeded = _kToolbarScreenPadding + _kToolbarHeight + _kToolbarContentDistance;
    final paddingTop = MediaQuery.of(context).padding.top;
    final availableHeight = globalEditableRegion.top + startTextSelectionPoint.point.dy - textLineHeight - paddingTop;
    final fitsAbove = closedToolbarHeightNeeded <= availableHeight;
    final anchor = Offset(
      ...
      children: <Widget>[
        CustomSingleChildLayout(
          ...
          child: _TextSelectionToolbar(
            ...
            handleShare: () => handleShare(delegate),
  ...
  Future<void> handleShare(TextSelectionDelegate delegate) async {
    final value = delegate.textEditingValue; // Snapshot the input before using `await`.
    // final ClipboardData data = await Clipboard.getData(Clipboard.kTextPlain);
    print('handleShare: ${value.selection.textInside(value.text)}');
    // if (data != null) {
  ...
  Widget buildHandle(BuildContext context, TextSelectionHandleType type, double textHeight) {
    ...
    final handleColor = theme.useTextSelectionTheme
...
final TextSelectionControls materialTextSelectionControls = AppTextSelectionControls();
